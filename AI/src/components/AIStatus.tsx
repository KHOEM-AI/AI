...
function Diag({ s }: { s: Shown }) {
  ...
  const reason = pickText(lang, s.reasonKm, s.reasonEn);
  return (
    <div className="status-card__diag">
      {reason && <div>{reason}</div>}
      {s.status === "TIMEOUT" ? (
        <div>{t.timeoutLabel}: {(s.ms ?? 0).toLocaleString("en-US")} ms</div>
      ) : (
      ...
      {s.module && <div>{t.moduleLabel}: {s.module}</div>}
      {s.error && <div className="status-card__err">{t.errorLabel}: {s.error}</div>}
      <div>{t.lastChecked}: {fmtTime(s.lastChecked)}</div>
      {bad && <div>{t.lastSuccessfulCheck}: {fmtTime(s.lastGood)}</div>}
    </div>
  );
}
...
export default function AIStatus({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="status-overlay" role="dialog" aria-label={t.navStatus}>
      <div className="status-panel">
        <div className="status-panel__head">
          <div>
            <div className="status-panel__ai-name">{AI_NAME}</div>
            <div className="status-panel__page-title">{pickText(lang, PAGE_TITLE_KM, PAGE_TITLE_EN)}</div>
          </div>
        ...
        <div className="status-panel__meta">
          ...
          <span>{t.lastChecked}: {fmtTime(system.lastChecked)}</span>
          {!GOOD.has(system.status) && <span>{t.lastSuccessfulCheck}: {fmtTime(system.lastGood)}</span>}
        </div>
